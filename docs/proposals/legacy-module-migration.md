...
> ✅ 当前版本：v1.0 | 下一版本：v1.5.0-rc1（含兼容层）
...
- ✅ 配置系统：`src/config.py` → `src/utils/config/`
- ✅ 核心处理器：已迁移至 `src/core/processors/`
- ✅ 服务层：组织到 `src/services/`

### ⚠️ 待迁移的遗留模块

| 遗留模块                     | 建议新位置                             | 依赖关系 | 风险等级 |
|--------------------------|-----------------------------------|------|------|
| `bilibili_downloader.py` | `src/services/download/bilibili_downloader.py` | 低    | 🟢 低 |
| `device_manager.py`      | `src/utils/device/device_manager.py`           | 中    | 🟡 中 |
| `extract_audio_text.py`  | `src/services/asr/transcriber.py` 或 `src/core/audio/extractor.py` | 高    | 🔴 高 |
| `logger.py`              | `src/utils/logging/logger.py`                  | 高    | 🔴 高 |
| `output_file_manager.py` | `src/core/export/file_manager.py`              | 中    | 🟡 中 |
| `process_history.py`     | `src/core/history/manager.py`                 | 中    | 🟡 中 |
| `subtitle_generator.py`  | `src/services/subtitle/generator.py`          | 中    | 🟡 中 |
| `task_manager.py`        | `src/utils/helpers/task_manager.py`           | 高    | 🔴 高 |
| `Timer.py`               | `src/utils/helpers/timer.py`                  | 低    | 🟢 低 |
| `core_process.py`        | 保留为向后兼容入口                           | 高    | 🔴 高 |
| `core_process_utils.py`  | 分散至相关模块                               | 中    | 🟡 中 |
