...
# ========== 步骤 3: 修改B站链接处理函数 ==========
def bilibili_process_wrapper_with_history(
    url, llm_api, temp, tokens, text_only, task_id
):
    """
    带历史检查的B站视频处理包装函数
...
def create_bilibili_tab_with_history():
    ...
    with gr.Tab("输入B站链接（带历史检查）"):
        ...
        history_info_box = gr.Textbox(
            label="历史记录信息", interactive=False, lines=6, visible=False
        )

        with gr.Row():
            llm_api_dropdown = gr.Dropdown(
                choices=LLM_SERVER_SUPPORTED, value=LLM_SERVER, label="选择LLM服务"
            )
            temp_slider = gr.Slider(
                0.0, 1.0, step=0.05, value=LLM_TEMPERATURE, label="Temperature"
            )
            token_slider = gr.Slider(
                100, 8000, step=100, value=LLM_MAX_TOKENS, label="Max Tokens"
            )
        ...
        with gr.Accordion("📝 处理结果文本", open=True):
            asr_text_output = gr.Textbox(
                label="ASR 识别文本", interactive=False, lines=8
            )
            polished_text_output = gr.Textbox(
                label="LLM 润色文本", interactive=False, lines=8
            )
            summary_text_output = gr.Textbox(
                label="文本摘要", interactive=False, lines=6
            )

        processing_time = gr.Textbox(
            label="下载+识别+润色用时（秒）", interactive=False
        )
        ...
        check_history_btn.click(
            fn=show_history, inputs=[bilibili_input], outputs=[history_info_box]
        )
        ...
        process_button.click(
            fn=generate_task_id, inputs=[], outputs=[task_id_state]
        ).then(
        ...
        stop_button.click(
            fn=stop_task_fn, inputs=[task_id_state], outputs=[stop_status]
        )
...
""")
        ...
        refresh_btn.click(fn=refresh_all, outputs=[stats_box, records_box])

        # 清空历史（危险操作）
        with gr.Accordion("⚠️ 危险操作", open=False):
            gr.Markdown(
                "**注意**: 清空历史记录不会删除实际输出文件，仅删除历史记录数据。"
            )
            clear_confirm = gr.Textbox(
                label="输入 'CONFIRM' 以确认清空所有历史记录", placeholder="CONFIRM"
            )
            ...
            clear_btn.click(
                fn=clear_history, inputs=[clear_confirm], outputs=[clear_status]
            )
