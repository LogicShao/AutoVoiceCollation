...
def test_process_bilibili_endpoint_end_to_end():
    ...
    assert h is not None and h.status_code == 200, (
        f"Health check failed: {getattr(h, 'status_code', None)}"
    )

    # choose a supported llm_api: prefer configured LLM_SERVER if valid, else pick the first supported
    chosen_llm = (
        config.llm.llm_server
        if config.llm.llm_server in config.llm.llm_server_supported
        else (config.llm.llm_server_supported[0] if config.llm.llm_server_supported else "")
    )
    payload = {
        "video_url": VIDEO_URL,
        "llm_api": chosen_llm,
    ...
    for attempt in range(1, post_retries + 1):
        try:
            ...
            resp = requests.post(
                f"{BASE_URL}/api/v1/process/bilibili", json=payload, timeout=(5, 120)
            )
            break
    ...
    try:
        ...
        pytest.fail(
            f"Failed to parse JSON from process response: {e} - body: {getattr(resp, 'text', None)}"
        )
